...
export default function ListRoom(props) {
  ...
  const handleJoinAndCallApi = function (id) {
    console.log(" id", id);
    Axios.post(
      `https://nana-wanna.herokuapp.com/api/group/register`,
      {
        group_id: id,
      },
      {
  ...
  function confirm(id) {
    ...
    Axios.get(`https://nana-wanna.herokuapp.com/api/group/delete?id=${id}`, {
      headers: {
          ...
          listRoomChat.map((data) => (
            <Col span={12} className="col-roomchat">
              <div className="item">
                ...
                <img
                  src={`https://nana-wanna.herokuapp.com/${data.image}`}
                  alt="logo"
                  style={{
                    maxWidth: "100%",
                    height: "150px",
                ...
                {data &&
                data.user_group &&
                data.user_group.findIndex(
                  (item) => inforUser && item.user_id === inforUser.id
                ) > -1 ? (
                  ...
                  <Button
                    className="join"
                    onClick={() => onClickShowConfirmJoin(data.id)}>
                    Tham gia
                  </Button>
                )}
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <button onClick={() => onClickHideConfirm()}>Hủy</button>
          </>
        }>
